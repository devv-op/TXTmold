  ...
  <div className="container-fluid">
    ...
    <div className="collapse navbar-collapse" id="navbarSupportedContent">
      <ul className="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        {/* <li className="nav-item">
          <Link className="nav-link" style={{color : 'white'}} to="/about">{props.aboutText}</Link>
        </li> */}
      </ul>
      <div className={`form-check form-switch text-${props.mode==='primary' ? 'dark' : 'primary'}`}>
        <input className="form-check-input" onClick={props.toggleMode} type="checkbox" id="flexSwitchCheckDefault"/>
        <label className="form-check-label" style={{color : 'white'}} htmlFor="flexSwitchCheckDefault">DarkMode</label>
      </div>
    </div>
  </div>
  </nav>
